...
// Resolves backend base URL for API calls
const getBackendBase = (): string => {
  if (typeof window === 'undefined') return '';
  const h = window.location.hostname;
  if (h === 'localhost' || h === '127.0.0.1') return 'http://localhost:8000';
  // Production: same origin but on the backend subdomain pattern used by Render
  return window.location.origin.replace('orvixa.onrender.com', 'orvixa-backend.onrender.com');
};

// ImageCard that fetches from the backend (Gemini Imagen → Pollinations fallback)
const BackendImageCard: React.FC<{ prompt: string }> = ({ prompt }) => {
  ...
  React.useEffect(() => {
    ...
    const fetchImage = async () => {
      setStatus('loading');
      try {
        // Read saved Gemini key from localStorage
        const stored = localStorage.getItem('orvixa_system_settings');
        let geminiKey: string | undefined;
        if (stored) {
          try { geminiKey = JSON.parse(stored)?.geminiApiKey || undefined; } catch { /* ignore */ }
        }
        // Also check orvixa_browser_preferences_v2
        const prefs = localStorage.getItem('orvixa_browser_preferences_v2');
        if (!geminiKey && prefs) {
          try { geminiKey = JSON.parse(prefs)?.customApiKey || undefined; } catch { /* ignore */ }
        }

        const base = getBackendBase();
        const resp = await fetch(`${base}/api/v1/media/generate-image`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ prompt, width: 800, height: 600, gemini_api_key: geminiKey || null }),
        });
  ...
  return (
    ...
    >
      <div style={{ padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)', backgroundColor: 'rgba(255,255,255,0.02)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ width: '100%', height: '180px', backgroundColor: 'rgba(10,10,20,0.9)', position: 'relative', overflow: 'hidden' }}>
        {status === 'loading' && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(10,10,20,0.9)' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', border: '3px solid rgba(99,102,241,0.2)', borderTopColor: 'var(--brand-primary)', animation: 'spin 0.8s linear infinite' }} />
          </div>
        )}
        {status === 'error' && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: '12px' }}>
            ...
            <button onClick={() => { setStatus('loading'); setRetryKey(k => k + 1); }} style={{ fontSize: '0.62rem', fontWeight: 800, color: 'var(--brand-primary)', background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.25)', borderRadius: '12px', padding: '4px 10px', cursor: 'pointer' }}>
              Retry
            </button>
          </div>
        )}
        {imgUrl && (
          <img
            src={imgUrl}
            alt={prompt}
            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', opacity: status === 'loaded' ? 1 : 0, transition: 'opacity 0.4s ease, transform 0.5s ease' }}
      ...
      <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', padding: '6px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', lineHeight: '1.35', fontStyle: 'italic' }}>
...
// Legacy static ImageCard (used for old-style ![alt](url) markdown images)
...
export const SafeMarkdownRenderer: React.FC<RendererComponentProps> = ({ payload }) => {
  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    // 6a. NEW: [IMAGE: descriptive prompt] tag — calls backend Gemini Imagen → Pollinations
    if (line.startsWith('[IMAGE:') && line.endsWith(']')) {
      flushList(idx);
      const prompt = line.slice(7, -1).trim();
      if (prompt) {
        renderedBlocks.push(
          <div
            key={`backend-img-${idx}`}
            style={{ margin: '12px 0', display: 'flex', flexWrap: 'wrap', gap: '10px' }}
          >
            <BackendImageCard prompt={prompt} />
          </div>
        );
        continue;
      }
    ...
    // 6b. Block-level standalone markdown image lines (e.g. lines that are ONLY ![alt](url))
